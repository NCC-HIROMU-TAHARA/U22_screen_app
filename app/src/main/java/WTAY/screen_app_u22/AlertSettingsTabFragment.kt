...
import WTAY.screen_app_u22.databinding.FragmentAlertSettingsTabBinding // FragmentAlertSettingsTabBindingに変更
import WTAY.screen_app_u22.databinding.DialogTimePickerBinding
import java.util.concurrent.TimeUnit

class AlertSettingsTabFragment : Fragment() {

    private var _binding: FragmentAlertSettingsTabBinding? = null
    private val binding get() = _binding!! // ここでバインディングオブジェクトを取得
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAlertSettingsTabBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as? MainActivity)?.setToolbarTitle("アラート設定")

        binding.fab.setOnClickListener { // binding.fab に変更
    ...
    private fun setupRecyclerView() {
        ...
        binding.recyclerView.layoutManager = LinearLayoutManager(requireContext()) // binding.recyclerView に変更
        binding.recyclerView.adapter = adapter // binding.recyclerView に変更
    }
    ...
    private fun loadAndDisplayAlertSettings() {
        ...
        if (allAlerts.isEmpty()) {
            binding.recyclerView.visibility = View.GONE // binding.recyclerView に変更
            binding.emptyView.visibility = View.VISIBLE // binding.emptyView に変更
        } else {
            binding.recyclerView.visibility = View.VISIBLE // binding.recyclerView に変更
            binding.emptyView.visibility = View.GONE // binding.emptyView に変更
